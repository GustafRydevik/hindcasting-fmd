...
pairs(resStan,pars=c("decay_start","decay_scale","decay_asym","gamma_shape","gamma_scale"))
...
herd_df_pred<-left_join(herd_df_pred,ind_df_pred%>%group_by(hcode)%>%summarise(probang_incidence=mean(Probang)),by="hcode")
plot(herd_df_pred$probang_incidence,summary(resStan,"monlast_pred")$summary[,"mean"])
